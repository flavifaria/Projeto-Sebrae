...
from amusicas import batalhas, parar_musica, vitoria
art = art_ascii()
console = Console()
...
class Jogador:
    ...
    def __init__(self, nome: str, max_hp: int, atc: int, max_mana: int, atm: int, gold: int, xp: int):
        ...
        self.itens_coletados_unicos = []
        
    def coletar_item_unico(self, item_nome: str, descricao_sucesso: str, descricao_falha: str):
        if item_nome in self.itens_coletados_unicos:
            console.print(f"[bold yellow]{descricao_falha}[/bold yellow]")
        else:
            self.inventario.append(item_nome)
            self.itens_coletados_unicos.append(item_nome)
            console.print(f"[bold green]{descricao_sucesso}[/bold green]")
        time.sleep(1.5)
    def adicionar_item_inventario(self, item_nome: str):
        self.inventario.append(item_nome)
        console.print(f"[bold green]Você adicionou {item_nome} ao seu inventário![/bold green]")
        time.sleep(1.5)
    ...
    def up(self):
        ...
        while True:
            ...
            table.add_column(f"[bold #EE82EE]Sua Experiencia: {self.xp}[/bold #EE82EE]")
            table.add_row(f"[bold green][1]Salde (Custo: {self.hp_upgrade_cost})[/bold green]")
            table.add_row(f"[bold blue][2]Estamina (Custo: {self.mana_upgrade_cost})[/bold blue]")
            table.add_row(f"[bold red][3]Ataque (Custo: {self.atc_upgrade_cost})[/bold red]")
            table.add_row(f"[bold #7B68EE][4]Especial (Custo: {self.atm_upgrade_cost})[/bold #7B68EE]")
            ...
            if escolha == '1':
                if self.xp >= self.hp_upgrade_cost:
                    ...
                    self.max_hp += 10
                    self.hp = self.max_hp 
                    self.hp_upgrade_cost = int(self.hp_upgrade_cost * 1.2) 
                ...
                if self.xp >= self.mana_upgrade_cost:
                    ...
                    self.max_mana += 10
                    self.mana = self.max_mana 
                    self.mana_upgrade_cost = int(self.mana_upgrade_cost * 1.2) 
                    console.print("[bold green]Estamina melhorada com sucesso![/bold green]")
                else:
                ...
                if self.xp >= self.atc_upgrade_cost:
                    ...
                    self.atc += 5
                    self.atc_upgrade_cost = int(self.atc_upgrade_cost * 1.2)
                ...
                if self.xp >= self.atm_upgrade_cost:
                    ...
                    self.atk_mana += 5
                    self.atm_upgrade_cost = int(self.atm_upgrade_cost * 1.2) 
                    console.print("[bold green]Especial melhorado com sucesso![/bold green]")
                else:
    ...
    def status(self):
        ...
        table.add_row(f"Saude: [#00FF00]{self.hp}[/#00FF00]/[#00FF00]{self.max_hp}[/#00FF00]")
        table.add_row(f"Estamina: [#7B68EE]{self.mana}[/#7B68EE]/[#7B68EE]{self.max_mana}[/#7B68EE]")
        table.add_row(f"Ataque: [#FF0000]{self.atc}[/#FF0000]")
        table.add_row(f"Especial: [#7B68EE]{self.atk_mana}[/#7B68EE]")
        table.add_row(f"Ouro: [#FFFF00]{self.gold}[/#FFFF00]")
        table.add_row(f"XP: [#4B0082]{self.xp}[/#4B0082]")
        equipped_items_str = "\n".join([f"{slot}: {item if item else 'Nenhum'}" for slot, item in self.equipado.items()])
                ...
                while True:
                    ...
                    if escolha_ops == '1':
                        while True:
                            ...
                            table = Table(box=box.HEAVY, style="blue")
                            table.add_column(f"[bold #FFD700]Dinheiro: {self.gold}[/bold #FFD700]")
                            table.add_row(f"[bold white][1]Pomada de Regeneração: 100\n[2]Refrigerante: 100\n[3]Sair[/bold white]")
                            print(table)
                            ...
                            if escolha_item == '1': 
                                ...
                                if self.gold >= cost:
                                    ...
                                    self.gold -= cost
                                    console.print(f"[bold green]Você comprou uma Pomada de Regeneração! Resta {self.gold} ouro.[/bold green]")
                                else:
                                    console.print("[bold red]Você não tem ouro suficiente para comprar uma Pomada de Regeneração.[/bold red]")
                                time.sleep(1.5)
                            elif escolha_item == '2':
                                cons = 100
                                if self.gold >= cons:
                                    self.inventario.append('Refrigerante')
                                    self.gold -= cons
                                    console.print(f"[bold green]Você comprou um Refrigerante! Resta {self.gold} ouro.[/bold green]")
                                else:
                                    console.print("[bold red]Você não tem ouro suficiente para comprar um Refrigerante.[/bold red]")
                                time.sleep(1.5)
                            elif escolha_item == '3':
                                break
                            else:
                        ...
                        while True:
                            ...
                            table = Table(box=box.HEAVY, style="blue")
                            table.add_column(f"[bold #FFD700]Dinheiro: {self.gold}[/bold #FFD700]")
                            table.add_row(f"[bold white][1]Raquete Eletrica: 1500\n[2]Esprei: 1500\n[3]Capacete: 1500\n[4]Escudo: 1500\n[5]Capuz: 1500\n[6]Sair[/bold white]")
                            print(table)
                            escolha_item1 = console.input("O que você deseja? ")
                            if escolha_item1 == '1':
                                cost = 1500
                                if self.gold >= cost:
                                    self.inventario.append('Raquete Eletrica')
                                    self.gold -= cost
                                    console.print(f"[bold green]Você comprou uma Raquete Eletrica! Resta {self.gold} ouro.[/bold green]")
                                else:
                                    console.print("[bold red]Você não tem ouro suficiente para comprar uma Raquete Eletrica.[/bold red]")
                                time.sleep(1.5)
                            elif escolha_item1 == '2':
                                cost = 1500
                                if self.gold >= cost:
                                    self.inventario.append('Esprei')
                                    self.gold -= cost
                                    console.print(f"[bold green]Você comprou um Esprei! Resta {self.gold} ouro.[/bold green]")
                                else:
                                    console.print("[bold red]Você não tem ouro suficiente para comprar um Esprei.[/bold red]")
                                time.sleep(1.5)
    ...
    def usar_item(self, item_nome: str):
        if item_nome == "Pomada de Regeneração":
        ...
        elif item_nome == 'Livro':
            if 'Livro' in self.inventario:
                self.xp *= 2
                self.inventario.remove('Livro')
                console.print(f'[bold blue]Você leu o Livro e ganhou conhecimento {self.xp}![/bold blue]')
            else:
                console.print("[bold red]Você não tem Poções de Mana no seu inventário.[/bold red]")
        elif item_nome == 'Refrigerante':
            if 'Refrigerante' in self.inventario:
                self.mana = min(self.max_mana, self.mana + 30)
                self.inventario.remove('Refrigerante')
                console.print('[bold blue]Você bebeu um Refrigerante e recuperou seu Especial![/bold blue]')
            else:
                console.print("[bold red]Você não tem Refrigerante no seu inventário.[/bold red]")
        elif item_nome == "Esprei":
            if "Esprei" in self.inventario:
                if self.equipado["Primeira Mão"] is None:
                    self.atk_mana *= 2 
                    self.inventario.remove("Esprei")
                    self.equipado["Primeira Mão"] = "Esprei"
                    console.print(f"[bold green]Você equipou o Esprei e seu ataque especial aumentou para {self.atk_mana}![/bold green]")
                else:
                    console.print(f"[bold yellow]Você já tem {self.equipado['Primeira Mão']} equipado. Desequipe-o primeiro para equipar o Esprei.[/bold yellow]")
            else:
                console.print("[bold red]Você não tem um Esprei no seu inventário.[/bold red]")
        elif item_nome == "Raquete Eletrica":
            if "Raquete Eletrica" in self.inventario:
                if self.equipado["Primeira Mão"] is None:
                    self.atc *= 2
                    self.inventario.remove("Raquete Eletrica")
                    self.equipado["Primeira Mão"] = "Raquete Eletrica"
                    console.print(f"[bold green]Você equipou a Raquete Eletrica e seu ataque aumentou em {self.atc}![/bold green]")
                else:
                    console.print(f"[bold yellow]Você já tem {self.equipado['Primeira Mão']} equipado. Desequipe-o primeiro para equipar a Raquete Eletrica.[/bold yellow]")
            else:
                console.print("[bold red]Você não tem uma Raquete Eletrica no seu inventário.[/bold red]")
            ...
            if "Escudo" in self.inventario:
                if self.equipado["Segunda Mão"] is None:
                    self.max_hp += 5 
                    self.hp = min(self.hp, self.max_hp) 
                    self.inventario.remove("Escudo")
                    self.equipado["Segunda Mão"] = "Escudo"
                    console.print("[bold green]Você equipou o Escudo e seu HP máximo aumentou em 5![/bold green]")
                else:
                    console.print(f"[bold yellow]Você já tem {self.equipado['Segunda Mão']} equipado. Desequipe-o primeiro para equipar o Escudo.[/bold yellow]")
            else:
    ...
    def desequipar_item(self, item_nome: str):
        if item_nome == "Cajado": 
            if self.equipado["Primeira Mão"] == "Cajado":
                self.atk_mana -= 5
                self.inventario.append("Cajado")
                self.equipado["Primeira Mão"] = None
            ...
            if self.equipado["Primeira Mão"] == "Espada Longa":
                self.atc -= 10
            ...
            if self.equipado["Segunda Mão"] == "Escudo":
                self.max_hp -= 5
                self.hp = min(self.hp, self.max_hp) 
                self.inventario.append("Escudo")
                self.equipado["Segunda Mão"] = None
            ...
            if self.equipado["Peito"] == "Peitoral":
                self.max_hp -= 25
                self.hp = min(self.hp, self.max_hp) 
    ...
    def exibir_menu_inventario(self):
        ...
        while True:
            ...
            if choice == '1':
                clear()
                utilizable_items = [item for item in self.inventario if item in ["Pomada de Regeneração",'Livro', "Refrigerante", "Raquete Eletrica", "Esprei", "Capacete", "Escudo", 'Capuz', 'Peitoral']]
    ...
    def menu(self):
        while True:
            clear()
            table = Table(style='white',box=box.HEAVY)
            table.add_column('MENU:')
            table.add_column('Fuções:')
            table.add_row(f'[bold]{art.pers}[/bold]',f'[bold]{art.menu}[/bold]')
            print(table)
            ops = console.input()
            if ops == '1':
                self.status()
...
def combat(player: Jogador, enemy: Inimigo):
    parar_musica()
    batalhas()
    console.print(f"--- [bold yellow]Um {enemy.name} apareceu![/bold yellow] ---")
    time.sleep(2)
    while player.hp > 0 and enemy.hp > 0:
        clear()
        console.print("--- [bold cyan]TURNO DE BATALHA[/bold cyan] ---")
        enemy.info()
        player.batalha_info()
        console.print("[bold green]Seu Turno![/bold green]")
        console.print("[bold red][1] Atacar[/bold red]")
        console.print("[bold magenta][2] Especial (10 de Estamina)[/bold magenta]")
        console.print("[bold yellow][3] Usar Poção[/bold yellow]")
        ...
        if choice == '1':
            ...
            if player.mana >= 10:
                ...
                console.print("[bold red]Estamina insuficiente![/bold red]")
        elif choice == '3':
            clear()
            utilizable_items_combat = [item for item in player.inventario if item in ["Pomada de Regeneração","Refrigerante"]]
            if not utilizable_items_combat:
                console.print("[bold yellow]Você não tem itens para usar no momento.[/bold yellow]")
                time.sleep(1.5)
                continue
            console.print("[bold magenta]Seu Inventário (Consumiveis):[/bold magenta]")
            for i, item in enumerate(utilizable_items_combat):
                console.print(f"[{i+1}] {item}")
            item_choice_combat = console.input("[bold blue]Escolha o número dos Consumiveis para usar (ou '0' para cancelar): [/bold blue]")
            try:
        ...
        if enemy.hp <= 0:
            break
        if player.hp > 0:
            console.print(f"[bold red]Turno do {enemy.name}![/bold red]")
...
|   |
 ...
 '''
    ...
    player1 = Jogador(nome, max_hp=100, atc=50, max_mana=50, atm=25, gold=100000000, xp=20)
    player1.inventario.append("Pomada de Regeneração")
    player1.inventario.append("Livro")
